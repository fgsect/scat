...
class DiagNrLogParser:
    ...
    def parse_nr_ml1_meas_db_update(self, pkt_header, pkt_body: bytes, args: dict):
        ...
        nr_cache = self.parent.nr_serving_cell[radio_id] if (self.parent and (cell_kv or meas_gsmtap) and radio_id in (0, 1)) else {}
        ...
        for layer in range(num_layers):
            ...
            # measured carrier — skip its all-zero serving-cell output (the kv row
            # and the GSMTAP signal report). Its neighbor cells are still reported.
            has_serving = meas_carrier_list.serv_cell_pci != 0xffff
            if cell_kv:
                if has_serving:
            ...
            if meas_gsmtap and has_serving:
                cp_pkts.append(util.build_signal_status_report(meas_carrier_list.raster_arfcn,
                    pci=meas_carrier_list.serv_cell_pci, band=nr_ident.get('band'),
                    ts_sec=ts_sec, ts_usec=pkt_ts.microsecond,
                    rsrp=self.parse_float_q7(meas_carrier_list.serv_rsrp_rx_0), is_nr=True))
            ...
            for cell in range(num_cells):
                ...
                if meas_gsmtap:
                    cp_pkts.append(util.build_signal_status_report(meas_carrier_list.raster_arfcn,
                        pci=cell_list.pci, ts_sec=ts_sec, ts_usec=pkt_ts.microsecond,
                        rsrp=self.parse_float_q7(cell_list.cell_quality_rsrp),
                        rsrq=self.parse_float_q7(cell_list.cell_quality_rsrq), is_nr=True))
