from diagcmd import create_log_config_set_mask as diag_port_config_gen
...
def dm_crc16(arr):
    ret = 0xffff
    for b in arr:
        ret = (ret >> 8) ^ crc16citt_table[(ret ^ b) & 0xff]
    return ret ^ 0xffff
...
"""
Hex string generator for DM port configuration of qualcomm chipset modems 
(used/created for manual testing purposes)
can use to test other modem brands e.g. Simcom, Telit, Quectel, etc.
"""

# hex string for manual testing of other modem brands/models 
def generate_diag_port_hex(subsys_id,*packet_types):
    packets = tuple([packet for packet in packet_types])
    return (hexlify(generate_packet(diag_port_config_gen(subsys_id,0x1000,packets))))
